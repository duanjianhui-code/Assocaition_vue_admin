<template>
  <div class="app-container">
    <!--第一部分，新建角色-->
    <el-button type="primary" @click="handleAddRole" v-if="checkPermission(createRoleRoles)">新建角色</el-button>
    ...
    <el-table :data="rolesList" style="width: 100%;margin-top:30px;" border>
      ...
      <el-table-column align="center" label="Operations">
        <template slot-scope="scope">
          <el-button type="primary" size="small" @click="handleEdit(scope)" v-if="checkPermission(updateRouteRoles)">编辑角色信息和分配角色路由</el-button>
          <el-button type="primary" size="small" @click="handleResourceEdit(scope)" v-if="checkPermission(updatePermRoles)">分配角色权限</el-button>
          <el-button type="danger" size="small" @click="handleDelete(scope)" v-if="checkPermission(deleteRoleRoles)">删除角色</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    handleResourceEdit(scope) {  // 响应分配角色权限按钮的点击
      let rowRole = deepClone(scope.row)
      this.role = rowRole
    ...
    async confirmRolePermission(){  // 角色权限对话框的提交按钮
      // 获取选中的树id
      let selectedKeys = this.$refs.permTree.getCheckedKeys()
